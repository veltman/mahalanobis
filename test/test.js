...
var expected2D = [1.2402,0.7602,0.1278,1.4657,1.6452,0.5444,2.231,1.5437,1.662,0.771],
    expected3D = [2.2104,1.3133,1.7528,0.6579,1.7931,1.4634,1.6439,1.9685,1.5632,1.5831];
...
// Test all
assert.deepEqual(roundTo(mahalanobis(test2D).all(),4), expected2D);
assert.deepEqual(roundTo(mahalanobis(test3D).all(),4), expected3D);

// Test individual 2D
var metric2D = mahalanobis(test2D);
var distances2D = test2D.map(metric2D.distance);
assert.deepEqual(roundTo(distances2D,4), expected2D);

// Test individual 2D
var metric3D = mahalanobis(test3D);
var distances3D = test3D.map(metric3D.distance);
assert.deepEqual(roundTo(distances3D,4), expected3D);
